									...
									<tbody>
										<c:forEach var="task" items="${listTasks}" varStatus="loop">
											<tr>
												...
												<td>${task.endDate}</td>
												<td><c:choose>
														<c:when test="${task.statusId == 1}">Chưa thực hiện</c:when>
														<c:when test="${task.statusId == 2}">Đang thực hiện</c:when>
														<c:when test="${task.statusId == 3}">Đã hoàn thành</c:when>
													</c:choose></td>
												<td><a
										...
										<c:if test="${empty listTasks}">
											<tr>
												<td colspan="7" class="text-center">Chưa có công việc
													nào</td>
											</tr>
										</c:if>
									</tbody>
